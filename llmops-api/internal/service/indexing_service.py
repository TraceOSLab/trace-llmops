        ...
        try:
            ...
            for node_id in node_ids:
                try:
                    collection.data.update(
                        uuid=node_id, properties={"document_enabled": document.enabled}
                    )
                except Exception as e:
            ...
            if document.enabled is True:
                ...
                self.keyword_table_service.add_keyword_table_from_ids(
                    dataset_id=document.dataset_id, segment_ids=enabled_segment_ids
                )
            else:
                # 从启用改为禁用 需要剔除相关关键词
                segment_ids = [id for id, _, _ in segments]
                self.keyword_table_service.delete_keyword_table_from_ids(
                    dataset_id=document.dataset_id, segment_ids=segment_ids
                )
        except Exception as e:
            # 记录失败日志并回退状态
            logging.exception(
                f"更改向量数据库文档启用状态失败，文档ID{document_id}，错误信息：{str(e)}"
            )
